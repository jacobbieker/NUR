...
def two_e(rand_gen, A, a, b, c):
    ...
    def create_haloes(number_of_haloes):
        """
        Creates a set number of haloes with 100 satellites each
        ...
        radii = np.concatenate(radii) # This makes the radii a single length list, for ease of use with binning
        return haloes, radii

    def calc_avg_satellites_per_bin(bin_values, bins, num_haloes):
        """
        Divide bin values by width of bins, then by number of haloes used to create it
        Gives average number of satellites per bin
        :param bin_values:
        :param bins:
        :return:
        """
    ...
    bin_values, bins, _ = plt.hist(radii, bins=log_bins) # Gets the bin values for the halo
    plt.cla()
    new_bin_values = calc_avg_satellites_per_bin(bin_values, bins, 1000)
